...
class AdvancedNLPAnalysis:
    ...
    def analyze_length_vs_accuracy(self):
        ...
        plt.title('Accuracy vs Text Length', fontsize=14, fontweight='bold')
        plt.legend()
        ...
        # Correlation
        print("\n" + "="*60)
        print("CORRELATION: Length × Accuracy")
        print("="*60)
    ...
    def analyze_typo_robustness(self, perturbed_predictions_dict):
        ...
        ax.bar(x - width/2, results_df['clean_accuracy'], width, 
               label='Clean Text', alpha=0.8)
        ax.bar(x + width/2, results_df['perturbed_accuracy'], width, 
               label='Text with Typos', alpha=0.8)
        
        ax.set_xlabel('Model', fontsize=12)
        ax.set_ylabel('Accuracy', fontsize=12)
        ax.set_title('Robustness to Typos', fontsize=14, fontweight='bold')
        ax.set_xticks(x)
    ...
    def analyze_emoji_impact(self):
        ...
        ax.bar(x - width/2, results_df['accuracy_with_emoji'], width,
               label='With Emojis', alpha=0.8)
        ax.bar(x + width/2, results_df['accuracy_without_emoji'], width,
               label='Without Emojis', alpha=0.8)
        
        ax.set_xlabel('Model', fontsize=12)
        ax.set_ylabel('Accuracy', fontsize=12)
        ax.set_title('Impact of Emojis on Accuracy', fontsize=14, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(results_df['model'])
        ax.legend()
        ax.grid(True, alpha=0.3, axis='y')
    ...
    def analyze_sarcasm(self, sarcasm_indices):
        ...
        Args:
            sarcasm_indices: List of indices of sarcastic reviews
        """
    ...
    def analyze_formality(self):
        ...
        slang_words = ['awesome', 'cool', 'sucks', 'lol', 'omg', 'gonna', 'wanna', 'gotta']
        ...
        plt.title('Accuracy by Formality Level', fontsize=14, fontweight='bold')
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    # For analyses that need additional data:
    # perturbed_preds = {...}  # Predictions on perturbed dataset
    # analyzer.analyze_typo_robustness(perturbed_preds)
    
    # sarcasm_indices = [10, 25, 47, ...]  # Manually annotated indices
