...
It generates a preliminary comparison table to validate the complete pipeline.
"""
...
def main():
    print("=" * 80)
    print("TASK 11 CHECKPOINT: VALIDATE ALL CLASSIFIERS")
    print("=" * 80)
    print()
    ...
    dist = data_loader.get_class_distribution()
    print(f"\n  Class distribution:")
    for split_name, counts in dist.items():
        total = sum(counts.values())
        neg = counts.get(0, counts.get('negative', 0))
        pos = counts.get(1, counts.get('positive', 0))
    ...
    # =========================================================================
    # METHOD 3: BERT (DistilBERT)
    # =========================================================================
    print("\n" + "=" * 80)
    print("METHOD 3: BERT (DISTILBERT)")
    print("=" * 80)
    ...
    comparison_df.to_csv('results/task11_comparison.csv', index=False)
    print("✓ Results saved to: results/task11_comparison.csv")
    ...
    for _, row in comparison_df.iterrows():
        ...
        for metric in ['accuracy', 'precision_macro', 'recall_macro', 'f1_macro']:
            ...
            if not (0.0 <= value <= 1.0):
                print(f"  ✗ {model} {metric} out of range: {value}")
                all_valid = False
    ...
    actual_models = set(comparison_df['model'].values)
    ...
    print("=" * 80)
    print()
    print("All classifiers have been validated successfully.")
    print("The pipeline is ready for multiple simulations (Task 13).")
    print()
